Attribute VB_Name = "uBQuickFillMod"
Public Function uBQuickFill(in_skillName, in_skillTime, in_timeArr, in_startRow)
    ...
    For Each r In arr
        ...
        For i = 0 To buffTime - 1 Step 1
            ...
            '填充
            If Cells(locationR + in_startRow, locationC + i - temp).Interior.ColorIndex = 37 Then
                doubleBuff = 1
            Else
                Cells(locationR + in_startRow, locationC + i - temp).Interior.ColorIndex = buffColor
            End If
            
            If doubleBuff = 1 Then
                Cells(locationR + in_startRow, locationC + i - temp) = "[2]"
            ElseIf i = 0 Then
